...
/**
 * * Example: The Hopfield Neural Network * * This is an example that implements
 * a Hopfield neural * network. This example network contains four fully *
 * connected neurons. This file, Hopfield, implements a * Swing interface into
 * the other two neural network * classes: Layer and Neuron. * * @author Jeff
 * Heaton * @version 1.0
 */
public class Hopfield extends JFrame implements ActionListener {
	...
	/**
	 * * The input pattern, used to either train * or run the neural network.
	 * When the network * is being trained, this is the training * data. When
	 * the neural network is to be ran * this is the input pattern.
	 */
	...
	/** * The clear button. Used to clear the weight * matrix. */
	JButton btnClear = new JButton("Clear");

	/**
	 * The train button. Used to train the * neural network.
	 */
	JButton btnTrain = new JButton("Train");

	/** * The run button. Used to run the neural * network. */
	JButton btnRun = new JButton("Run");

	/**
	 * * Constructor, create all of the components and position * the JFrame to
	...
	public Hopfield() {
		...
		// row 
		c.anchor = GridBagConstraints.NORTHWEST; content.add( new
		JLabel("Weight Matrix for the Hopfield Neural Network:"),c);

		// Weight matrix 
		c.anchor = GridBagConstraints.CENTER; c.gridwidth =
		...
		pack(); Toolkit toolkit =
		 Toolkit.getDefaultToolkit(); Dimension d = toolkit.getScreenSize();
	...
	/**
	 * * Used to dispatch events from the buttons * to the handler methods. *
	 * * @param e The event
	 */
	...
	protected void run() {
		...
		for (int row = 0; row < NETWORK_SIZE; row++)
			for (int col = 0; col < NETWORK_SIZE; col++)
				wt[row][col] = Integer.parseInt(matrix[row][col].getText().trim());
		for (int row = 0; row < NETWORK_SIZE; row++) {
	...
	/**
	 * * Called to train the weight matrix based on the * current input pattern.
	 */
	protected void train() {
		...
		for (int row = 0; row < NETWORK_SIZE; row++)
			for (int col = 0; col < NETWORK_SIZE; col++) {
				int i = Integer.parseInt(matrix[row][col].getText().trim());
	...
	/**
	 * * Main program entry point, display the * frame. * * @param args Command
	 * line arguments are not used
	 */
